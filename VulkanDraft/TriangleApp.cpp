...
void TriangleApp::drawFrame()
{
	...
	VkSemaphore signalSemaphores[] = { m_RendererFinishedSemaphores[m_CurrentFrame] };
...
bool TriangleApp::isDeviceSuitable(VkPhysicalDevice device)
{
	...
	bool extensionsSupported = checkExtensionDeviceSupport(device);
...
bool TriangleApp::checkExtensionDeviceSupport(VkPhysicalDevice device)
{
...
void TriangleApp::createGraphicPipeline()
{
...
void TriangleApp::createSyncObjects()
{
	...
	m_RendererFinishedSemaphores.resize(MAX_FRAMES_IN_FLIGHT);
	...
	for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++)
	{
		if (vkCreateSemaphore(m_Device, &semaphoreInfo, nullptr, &m_ImageAvailableSemaphores[i]) != VK_SUCCESS ||
			vkCreateSemaphore(m_Device, &semaphoreInfo, nullptr, &m_RendererFinishedSemaphores[i]) != VK_SUCCESS ||
...
void TriangleApp::createDescriptorSets()
{
	...
	{
		throw std::runtime_error("Failedto allocate descriptor sets!");
	}
...
void TriangleApp::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout)
{
	...
	if (newLayout == VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL)
	{
		...
		if (hasStencillComponent(format))
		{
...
VkImageView TriangleApp::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlag)
{
...
VkFormat TriangleApp::findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features)
{
	...
	throw std::runtime_error("Failed to fing supported format!");
}
...
bool TriangleApp::hasStencillComponent(VkFormat format)
{
